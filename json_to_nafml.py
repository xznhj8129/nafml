...
Usage:
    python json_to_nafml.py --out inav_msp.yaml
"""
...
# sections whose entries are long enough to want breathing room between them
...
def render(document: dict) -> str:
    ...
    for section, body in document.items():
        if section in ("version", "package") or not body:
            continue
        out.append("")
...
def main() -> int:
    ...
    document: dict = {"version": 1, "package": "inav.msp"}
    if converter.constants:
        document["constants"] = dict(sorted(converter.constants.items()))
    document["aliases"] = dict(OPAQUE)
    if enums:
        document["enums"] = enums
    if bitmasks:
        document["bitmasks"] = bitmasks
    document["messages"] = messages

    args.out.write_text(render(document))
